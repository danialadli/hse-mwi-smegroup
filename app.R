...
server <- function(input, output, session) {
  ...
  output$main_ui <- renderUI({
    # Get current language from input or default
    lang <- if(is.null(input$selected_language)) "en" else input$selected_language
    
    # Set the translation language
    i18n$set_translation_language(lang)
    
    # Render the main UI with all required variables
    get_main_ui(i18n, lang, index_types, f_st, overall, mwi_toolkit_order, show_mitre)
  })
  # ----------------------------------
  # ----------------------------------
